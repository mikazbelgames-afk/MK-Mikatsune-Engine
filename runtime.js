...
async function boot(){
  ...
  // runtime.html?project=https://.../proyecto.json
  if(externalProject){
    loaded=await loadFromProjectURL(externalProject);
  }

  // Nuevo puente editor → runtime.
  // Si no se entrega un JSON por URL, usa automáticamente
  // el último proyecto guardado por el editor en IndexedDB.
  if(!loaded){
    try{
      loaded=await loadProjectData(await loadCurrentProject());
    }catch(err){
      console.error('No se pudo leer el proyecto actual:',err);
    }
  }
...
function frame(now){
  ...
  if(Math.random()<.0018 && blinkStart===null){
    blinkStart=now;
  }
...
window.Mikatsune={
  ...
  async reload(){
    await loadProjectData(await loadCurrentProject());
  }
};
